...
impl ChannelDriver for PjsipChannelDriver {
    ...
    // REVIEW-M3 MINOR-1: DTMF logging must never include the digit VALUE --
    // entered digits are PIN material (log that a digit happened, not which).
    async fn send_digit_begin(&self, channel: &mut Channel, _digit: char) -> AsteriskResult<()> {
        let priv_data = self
            .get_private(channel.unique_id.as_str())
            .ok_or_else(|| AsteriskError::NotFound(channel.name.clone()))?;
        ...
        match priv_data.dtmf_mode {
            DtmfMode::Rfc2833 | DtmfMode::Auto => {
                ...
                debug!(call_id = %priv_data.call_id, "DTMF begin (RFC 2833)");
            }
            DtmfMode::Info => {
                // SIP INFO -- begin is a no-op, we send on digit_end.
                debug!(call_id = %priv_data.call_id, "DTMF begin (SIP INFO, deferred)");
            }
            DtmfMode::Inband => {
    ...
    async fn send_digit_end(
        ...
        match priv_data.dtmf_mode {
            DtmfMode::Rfc2833 | DtmfMode::Auto => {
                ...
                debug!(call_id = %priv_data.call_id, duration, "DTMF end (RFC 2833)");
            }
            DtmfMode::Info => {
                // Send SIP INFO with application/dtmf-relay.
                debug!(call_id = %priv_data.call_id, duration, "DTMF end (SIP INFO)");
            }
            DtmfMode::Inband => {
